...
The UI follows a **clean, modern SaaS** aesthetic inspired by Linear, Notion, and Vercel.
...
|---|---|---|
| `--bg-base` | `#f8f9fc` | Page background (cool off-white) |
| `--bg-surface` | `#ffffff` | Cards, panels, inputs |
| `--primary` | `#3B82F6` | Brand blue — links, active states, accents |
...
| `--destructive` | `#EF4444` | Delete actions (red-500) |
| `--error-bg/border/text` | reds | Error states |
| `--success-bg/text` | greens | Success states |
| `--card-shadow` | `0 1px 3px` | Soft blur shadow |
| `--card-shadow-hover` | `0 4px 12px` | Elevated hover shadow |

Note colors for cards are defined in `src/lib/utils/colors.ts`.

### Typography

- **Body/UI**: `Inter` (sans-serif) — clean, modern UI font
- **Code blocks**: `JetBrains Mono` (monospace) — reserved for `.tiptap code`, `.prose code` only

### Visual rules

- **Shadows**: Soft, blurred elevation shadows (`0 1px 3px` normal, `0 4px 12px` hover). Never use hard-offset shadows.
- **Corners**: `rounded-lg` (8px) everywhere. Color picker circles use `rounded-full`.
- **Borders**: Light, subtle borders using `--border` and `--border-subtle`. Use shadow/elevation to separate surfaces where appropriate.
- **Animations**: Fast and crisp (150ms `ease-out`). No spring physics, no bounce.
- **Checkboxes**: Use `accent-color: var(--primary)` globally.
...
- Test database: `./data/test-slabs.db` (cleaned via `global-setup.ts`)
